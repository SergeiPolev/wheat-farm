...
// See : https://gist.github.com/Cyanilux/e7afdc5c65094bfd0827467f8e4c3c54
	...
	void vertInstancingGroundMatrices(inout float4x4 objectToWorld, out float4x4 worldToObject) {
		MeshProperties data = _PerInstanceData[unity_InstanceID];

		objectToWorld = mul(objectToWorld, data.gr);
...
void GetUV_float(float ID, out float4 UV)
{
    UV = _PerInstanceData[ID].uv;
}
...
{
	color = _PerInstanceData[ID].color;
}
#endif
